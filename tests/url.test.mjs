...
test("getWebsiteUrl normalizes website-like input", () => {
  ...
  assert.equal(getWebsiteUrl("localhost:5173"), "https://localhost:5173/");
  assert.equal(getWebsiteUrl("192.168.1.10:8080"), "https://192.168.1.10:8080/");
});

test("getWebsiteUrl resolves exact website aliases", () => {
  const aliases = [
    { aliases: ["jira", "current board"], url: "https://example.atlassian.net/jira" },
    { aliases: ["docs"], url: "https://docs.example.com" }
  ];

  assert.equal(getWebsiteUrl("jira", aliases), "https://example.atlassian.net/jira");
  assert.equal(getWebsiteUrl("Current Board", aliases), "https://example.atlassian.net/jira");
  assert.equal(getWebsiteUrl("docs", aliases), "https://docs.example.com/");
});
...
test("getWebsiteUrl rejects search-like input", () => {
  ...
  assert.equal(getWebsiteUrl("jira"), null);
